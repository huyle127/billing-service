...
import { ALLOCATION_KEYS } from '../src/billing/billing.constants';
import { AddonPurchaseService } from '../src/billing/services/addon-purchase.service';
...
import { PAYMENT_STATUSES, STRIPE_API_VERSION } from '../src/billing/stripe/stripe.constants';
...
import { WEBHOOK_EVENT_TYPES } from '../src/billing/webhook/webhook.constants';
...
describe('buying add-on credits', () => {
  ...
  it('refuses a frozen wallet before Stripe is called', async () => {
    ...
    const thrown = await purchases
      .purchase(user.id, { packageCode: SMALL })
      .catch((error: unknown) => error as DomainException);

    expect(thrown.code).toBe('WALLET_FROZEN');
