# subscription-provisioning Specification

## Purpose
TBD - created by archiving change build-registration-provisioning. Update Purpose after archive.
## Requirements
...
`POST /v1/internal/provisioning/run` SHALL invoke the same sweep the in-process schedule invokes, so
an external scheduler can drive it without a logic change.
