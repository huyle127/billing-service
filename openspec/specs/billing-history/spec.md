...
The single read that answers "what has happened on my account". History is derived, never stored:
`PaymentTransaction`, `CreditTransaction`, and `SubscriptionEvent` are merged on every request and
nothing writes to a history table, so there is no second source of truth that can disagree with the
ledgers. One endpoint carries all three sources rather than three endpoints carrying one each,
because splitting it would push the merging work onto every client. The merge crosses a module
boundary — `credit` owns `CreditTransaction` — so the credit slice is read through `CreditService`
rather than by a SQL union, and the page is assembled in application code.
